...
class control extends session
{
    ...
    public function __construct()
    {
		...
		// HTML template laden
		if($classname!='ajaxControl') $this->loadView("template");
	...
	public function loadModel($model)
	{
		if(file_exists("model/".$model.".php"))
		{
			...
			if(class_exists($classname)) $this->$classname = new $classname;
		}
	}
	...
	private function formfill()
	{
		foreach($_POST as $key=>$value)
		{
			...
			if(is_int($value)) $this->replaceView('POST_'.$key.'_'.$value,'selected');
		}
		foreach($_GET as $key=>$value)
		{
